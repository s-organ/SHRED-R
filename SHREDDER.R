...
# 1) Conservative p-values 

parents_pval = c()
for(i in 1:nrow(hypothesis)){
  cat(paste0(round(i / (nrow(hypothesis)) * 100), '% Calculating adjusted p-values completed'))
  current_set = c(hypothesis$set[i])
  current_set = as.numeric(which(mat2[current_set,] != 0))
  parents = c()
  for(j in 1:nrow(mat2)){
    parents[j] =  all(current_set %in% c(as.numeric(which(mat2[j,] != 0))))
  }
  parents = which(parents == TRUE)
  hypothesis_parents = hypothesis %>% filter(set %in% c(parents))
  p_parents = which.max(hypothesis_parents$p_val)
  parents_pval[i] = hypothesis_parents$p[p_parents]
  if (i == (nrow(hypothesis))) cat(': Done')
  else cat('\014')
}

hypothesis$p_val = parents_pval

# 2) Order hypotheses by ascending conservative p-value
...
# 3) get minimal sum of weights (sigma) at each p-value
...
threshold <- hypothesis$p_val * (p / q)
...
# 5) Final rejection: keep only minimal selected sets
